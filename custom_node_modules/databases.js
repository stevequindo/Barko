...
exports.getCountries = function(userReq) {
	// /flags.findFlagUrlByCountryName(nzCountry.countryName);
	// TODO: Fix country image loading
	// Get all the countries associated with the given user and aggregate into different countries

	console.log(flags.findFlagUrlByCountryName("New Zealand"));
	if (userReq.local.role === "staff") {
		let results = Container
				...
				.addFields({
					flagUrl: flags.findFlagUrlByCountryName("$departureCountry")
				});
		...
		return Container
			...
			.group({countryName: "$targetCountry",  numOfContainers: {$sum : 1}})
			.addFields({
				flagUrl: flags.findFlagUrlByCountryName("$departureCountry")
			});
	}
