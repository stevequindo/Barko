...
exports.findStatus = function(trackingNum) {
	...
	Returns:
		status: Status of the transaction in String
	*/
	return new Promise(function (resolve, reject) {
		Transaction.find({trackingNumber: trackingNum}, 'status', function(err, docs) {
			if (docs.length > 0) {
				let status = docs[0].status;
				console.log(`Retrieved status ${status}. Resolving...`);
				resolve(status);
			} else {
				console.log(`Status is empty. Rejecting...`);
				reject(status);
			}
		});
	});
